...
namespace cora::compiler
{
    namespace runtime
    {
        ...
        void Interpreter::Run(const std::string &source)
        {
            ...
            semantic::ValidateProgram(program, m_FileName, m_ModuleName);
            Execute(program);
        ...
        void Interpreter::RegisterModule(const std::string &name, const std::shared_ptr<Object> &moduleObject, bool constant)
        {
            if (name.empty() || !moduleObject)
            {
                return;
            }

            builtin::RegisterModule(m_GlobalScope, name, moduleObject, constant);
            m_ModuleCache[name] = moduleObject;
        }
        ...
        std::shared_ptr<Object> Interpreter::LoadModuleFromFile(const std::string &modulePath, const std::filesystem::path &path)
        {
            ...
            semantic::ValidateProgram(program, path.string(), modulePath);
        ...
        void Interpreter::ExecStmt(Statement *stmt)
        {
            ...
            if (auto *assign = dynamic_cast<ast::Assignment *>(stmt))
            {
                ...
                if (auto *memberTarget = dynamic_cast<ast::MemberExpr *>(assign->GetTarget()))
                {
                    ...
                    if (object->IsConstMember(memberTarget->GetMember()))
                    {
                        if (object->IsConstMemberInitialized(memberTarget->GetMember()))
                        {
                            RaiseRuntimeError("Cannot assign to constant member: " + memberTarget->GetMember());
                        }

                        bool isThisObject = false;
                        Scope *thisScope = m_CurrentScope->ResolveVariable("this");
                        if (thisScope != nullptr)
                        {
                            Variable *thisVariable = thisScope->GetVariable("this");
                            if (thisVariable != nullptr && thisVariable->GetValue() != nullptr && thisVariable->GetValue()->IsObject())
                            {
                                auto thisObject = thisVariable->GetValue()->AsObject();
                                isThisObject = thisObject && thisObject.get() == object.get();
                            }
                        }

                        const bool inConstructor = !m_FunctionStack.empty() &&
                                                   m_FunctionStack.back() == "__init__" &&
                                                   !m_ClassStack.empty() &&
                                                   m_ClassStack.back() == object->className;
            ...
            if (auto *classDecl = dynamic_cast<ast::ClassDeclStmt *>(stmt))
            {
                ...
                std::unordered_map<std::string, bool> methodVisibility;
                for (ast::FunctionDeclStmt *method : classDecl->GetMethods())
                {
                    const std::string methodName = method->GetName();
                    methodVisibility[methodName] = methodVisibility[methodName] || method->GetAccessModifier() == ast::AccessModifier::Public;
                ...
                for (const auto &visibility : methodVisibility)
                {
                    klass->SetMemberVisibility(visibility.first, !visibility.second);
                }
